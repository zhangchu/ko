...
class Ko_Data_Storage extends Ko_Busi_Api
{
    ...
    private function _aGetImageGpsExif($exif)
    {
        if ((is_array($exif['GPSLatitude']) || strlen($exif['GPSLatitude']))
            ...
            if (is_array($exif['GPSLatitudeRef']) && isset($exif['GPSLatitudeRef']['val'])) {
                $GPSLatitudeRef = $exif['GPSLatitudeRef']['val'];
            } else {
                $GPSLatitudeRef = $exif['GPSLatitudeRef'];
            }
            if (is_array($exif['GPSLongitudeRef']) && isset($exif['GPSLongitudeRef']['val'])) {
                $GPSLongitudeRef = $exif['GPSLongitudeRef']['val'];
            } else {
                $GPSLongitudeRef = $exif['GPSLongitudeRef'];
            }

            if (is_array($exif['GPSLatitude'])) {
                if (isset($exif['GPSLatitude']['val'])) {
                    $GPSLatitude = $exif['GPSLatitude'] = explode(',', $exif['GPSLatitude']['val']);
                } else {
                    $GPSLatitude = $exif['GPSLatitude'];
                }
            } else {
                $GPSLatitude = $exif['GPSLatitude'] = explode(',', $exif['GPSLatitude']);
            }
            if (is_array($exif['GPSLongitude'])) {
                if (isset($exif['GPSLongitude']['val'])) {
                    $GPSLongitude = $exif['GPSLongitude'] = explode(',', $exif['GPSLongitude']['val']);
                } else {
                    $GPSLongitude = $exif['GPSLatitude'];
                }
            } else {
                $GPSLongitude = $exif['GPSLongitude'] = explode(',', $exif['GPSLongitude']);
            }

            return array(
                'lat' => $this->_aGetImageGps($GPSLatitude, $GPSLatitudeRef),
                'lng' => $this->_aGetImageGps($GPSLongitude, $GPSLongitudeRef),
            );
        }
        return false;
    }

    private function _aGetImageGps($coordinate, $hemisphere)
    {
        foreach ($coordinate as &$v) {
            $part = explode('/', trim($v), 2);
            if (2 == count($part)) {
                $v = floatval($part[0]) / floatval($part[1]);
            } else {
                $v = floatval($part[0]);
            }
        }
